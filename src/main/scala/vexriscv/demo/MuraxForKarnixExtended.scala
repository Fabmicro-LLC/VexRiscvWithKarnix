...
case class MuraxForKarnixExtendedConfig(coreFrequency : HertzNumber,
...
object MuraxForKarnixExtendedConfig{
  def default : MuraxForKarnixExtendedConfig = default(false, false)
  def default(withXip : Boolean = false, bigEndian : Boolean = false) =  MuraxForKarnixExtendedConfig(
...
case class MuraxForKarnixExtendedTopLevel() extends Component{
    ...
    val murax = MuraxForKarnixExtended(MuraxForKarnixExtendedConfig.default(withXip = false).copy(
		...
		onChipRamHexFile = "MuraxForKarnixExtendedTopLevel_random.hex"
...
object MuraxForKarnixExtendedVerilogSim {
  def main(args: Array[String]) {
